...
description: There's an obscure way of getting an API for reading a Google Sheet, that doesn't require authentication or complicated permissions. All you need to do is publish the spreadsheet, and use this immediately-updating endpoint to read the contents of the spreadsheet.
...
Here's some example code I use to format the data, using the [`fetch`](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) method in JavaScript: 
    ...
    for(const row of rows) {
      ...
      for(const key in row) {
        if(key.startsWith("gsx$")) {
          ...
          // The actual row names from your spreadsheet are formatted like "gsx$title".
          // Therefore, we need to find keys in this object that start with "gsx$", and then strip that out to get the actual row name
