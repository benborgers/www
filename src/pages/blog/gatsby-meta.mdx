...
description: I include the og:type, og:site_name, og:title, og:description, and og:image meta tags in my Gatsby sites using a reusable `Head` component. You could also add equivalent twitter:* tags, but Twitter falls back on og:* tags, so there's no need to include both. 
...
*By the way, I explained why I use `react-helmet-async` instead of `react-helmet` in [this post](https://benborgers.com/blog/react-helmet-async).*
