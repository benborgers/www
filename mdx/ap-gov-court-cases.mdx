...
export function Case({ name, bullets }) {
    return (
        <Disclosure as="div" className="bg-blue-50 border border-blue-500 p-4 rounded-lg">
            {({ open }) => (
                <>
                    <Disclosure.Button className="flex items-center space-x-2 text-blue-600 cursor-pointer focus:outline-none" style={{ marginTop: '0 !important' }}>
                        <ChevronRightIcon className={`${open ? 'transform rotate-90' : ''} transition-all h-5`} />
